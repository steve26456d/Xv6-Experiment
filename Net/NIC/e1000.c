...
e1000_recv(void)
{
  ...
  //
  // printf("call to e1000_receive\n");
  int tail;
  int next;
  while(1){
    tail = regs[E1000_RDT];
    next = (tail + 1) % RX_RING_SIZE;
    if((rx_ring[next].status & E1000_RXD_STAT_DD) == 0)
      break;
    // printf("there\n");
    net_rx(rx_bufs[next],rx_ring[next].length);
    // printf("here\n");
    if((rx_bufs[next] = kalloc()) == 0)
      panic("e1000_recv alloc error\n");
